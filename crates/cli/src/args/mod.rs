...
pub(crate) async fn run(args: Args) -> Result<(), AnyError> {
    ...
    if let Some(input) = args.input {
        ...
        return Ok(tokio_convert(input, output, out_fmt).await?);
    }
    ...
    if let Some(command) = args.subcommand {
        Ok(match command {
            ...
            SubCommands::AnnoDump(args) => self::hkanno::dump(&args).await?,
            SubCommands::AnnoUpdate(args) => self::hkanno::update(&args).await?,
...
pub(crate) enum SubCommands {
    ...
    /// Extract annotations from HKX files to text files
    AnnoDump(hkanno::DumpArgs),
    /// Update HKX files with annotations from text files
    AnnoUpdate(hkanno::UpdateArgs),
