...
OVERVIEW
--------
hkxc-anno-cli is a command-line tool for extracting and updating animation
annotations in Havok HKX files used by Skyrim and other Bethesda games. It
wraps the hkxc.exe converter to provide a streamlined workflow for managing
animation event annotations.

Animation annotations are time-stamped events embedded in HKX files that
trigger game behaviors like weapon swings, hit detection, sound effects,
and state changes during animation playback.
...
- "Annotation file not found" - Check that .txt file exists in annos folder
  with matching path structure
- "XML file not found in cache" - The HKX file may be corrupted or not a
